...
fun main(){
    Solution().solve(File("day07-part1/src/input.txt").readLines())
}

class Solution{
    private val strength = mapOf('A' to 1, 'K' to 2, 'Q' to 3, 'J' to 4, 'T' to 5, '9' to 6, '8' to 7, '7' to 8, '6' to 9, '5' to 10, '4' to 11, '3' to 12, '2' to 13)
    ...
    private fun getHandType(cards: String): HandType{
        ...
        if(distinct.size == 1)
            ...
            if(cards.count { it == distinct[0] } == 1 || cards.count { it == distinct[0] } == 4)
                return HandType.FourOfAKind
            else if(cards.count { it == distinct[0] } == 2 || cards.count { it == distinct[0] } == 3)
                return HandType.FullHouse
        }
        else if(distinct.size == 3){
            if(distinct.any { d -> cards.count { c -> c == d } == 3})
                return HandType.ThreeOfAKind
            else if (distinct.any { d -> cards.count { c -> c == d } == 2})
